"""Pure rendering of the Thai stock summary into email HTML / plain text.

No secrets, no network, no SMTP — just turns the structured JSON summary
(see /daily-stocks) into an email body. Shared by:
  - render_email.py  (prints HTML or text; used by the Gmail-connector path)
  - send_email.py    (legacy SMTP fallback)
"""
...
def subject_for(data):
    ...
    return f"📈 สรุปหุ้นเทค/ชิป/AI ประจำวัน {date_str}"
